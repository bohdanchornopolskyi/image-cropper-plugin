...
export async function GET(request: Request) {
  ...
  // ?id= comes from the admin Preview button; without it, fall back to the first post.
  const id = new URL(request.url).searchParams.get('id')
  const { docs } = await payload.find({
    collection: 'posts',
    depth: 1,
    limit: 1,
    ...(id ? { where: { id: { equals: id } } } : {}),
  })
  ...
  // What the browser actually gets when no focal point is set.
  const pos = focalPosition ?? '50% 50%'
  const coverStyle = `width:100%;height:100%;object-fit:cover;object-position:${pos};display:block;`
...
<body>
  ...
  <p style="color:#888;font-size:.85rem;margin-bottom:1rem;">
    ...
    object-position: ${pos}</code>. Now actually resize the browser window
  ...
  ${
    ...
    <img src="${focalRawUrl}" alt="Hero banner" style="${coverStyle}">
  </div>
  <p style="color:#888;font-size:.8rem;margin-top:.5rem;">object-position: <code>${pos}</code></p>`
  ...
  <p style="color:#888;font-size:.85rem;margin-bottom:1rem;">
    ...
    Each one applies <code>object-fit: cover; object-position: ${pos}</code>,
  ...
  <div class="grid">
    ...
    <div class="card">
      ${
        focalRawUrl
          ? `<div style="aspect-ratio:${ratio};overflow:hidden;"><img src="${focalRawUrl}" alt="${label}" loading="lazy" style="${coverStyle}"></div>`
  ...
  <div style="position:relative;display:inline-block;max-width:100%;background:#000;border:1px solid #2a2a2a;border-radius:8px;overflow:hidden;">
    ...
    ${
      heroRawImage &&
      typeof heroRawImage.focalX === 'number' &&
      typeof heroRawImage.focalY === 'number'
